...
const TableOfContents = ({ title, className, style }) => {
  ...
  const handleTocKeyDown = e => {
    ...
    // Handle escape key to close TOC or mobile menu
    handleEscapeKey(e, () => {
      if (isMobileMenuOpen) {
        setIsMobileMenuOpen(false)
      } else {
  ...
  const mobileToggleHandlers = createKeyboardClickHandlers(toggleMobileMenu, {
    role: "button",
  })

  return (
    <>
      {/* Mobile floating TOC button - only visible on mobile */}
      {headings.length > 0 && (
        <div
