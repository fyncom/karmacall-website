...
const CashOut = () => {
  ...
  const buildSolanaPaymentUrl = (amount, planName) => {
    ...
    params.set("amount", normalizedAmount.toFixed(6))
  ...
  const handleSolanaDeposit = async (amount, planName) => {
    ...
    if (solanaDepositMode === "qr") {
      await showDepositQr(normalizedAmount, planName)
      ReactGA.event({
        category: "solana",
    ...
    if (!solanaWalletAddress) {
      await showDepositQr(normalizedAmount, planName, "We didn't detect a browser wallet on this device.")
      ReactGA.event({
        category: "solana",
    ...
    try {
      ...
      await showDepositQr(normalizedAmount, planName, "We could not complete the deposit with the browser wallet.")
      ReactGA.event({
        category: "solana",
        action: "deposit_qr_fallback",
        label: planName,
        value: normalizedAmount,
      })
    } finally {
          ...
          <div style={{ marginTop: "40px", borderTop: "1px solid #ddd", paddingTop: "20px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "16px", marginBottom: "24px" }}>
              ...
              <div style={{ border: "2px solid #e5e7eb", borderRadius: "8px", padding: "16px", backgroundColor: "#f9fafb" }}>
                ...
                <a href="https://play.google.com/store/apps/details?id=com.fyncom.robocash" target="_blank" rel="noreferrer">
                  <button
                    style={{
                      width: "100%",
                      background: "#6b7280",
              ...
              <div style={{ marginTop: "24px", border: "1px solid #dbeafe", borderRadius: "12px", padding: "20px", backgroundColor: "#f8fafc" }}>
                ...
                {depositQrData.qrDataUrl && (
                  <div style={{ textAlign: "center", marginBottom: "12px" }}>
                    <img src={depositQrData.qrDataUrl} alt="Solana Pay QR code" style={{ maxWidth: "220px", width: "100%" }} />
                  </div>
                )}
                <p style={{ fontSize: "13px", wordBreak: "break-all", marginBottom: "12px" }}>
                  Link:{" "}
                  <a href={depositQrData.solanaUrl} target="_blank" rel="noreferrer">
                    {depositQrData.solanaUrl}
                  </a>
                </p>
                <button
